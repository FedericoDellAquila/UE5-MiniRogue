...
AGameplayGameMode::AGameplayGameMode(const FObjectInitializer& ObjectInitializer)
	...
	, Die(nullptr)
{
	if (IsValid(SimulationWorldComponent) == false)
	{
		SimulationWorldComponent = CreateDefaultSubobject<USimulationWorldComponent>(TEXT("SimulationWorldComponent"));
	}
}
...
// void AGameplayGameMode::SimulateRoll(TArray<FTransformPhysicsSteps>& OutAnimationPaths, int32 NumDice)
// {
// 	UWorld* SimWorld = CreateSimulationWorld();
// 	if (IsValid(SimWorld) == false)
// 	{
// 		return;
// 	}
//
...
// 	{
// 		FTransformPhysicsSteps NewPath;
// 		NewPath.DieID = i;
...
// 	SimWorld->InitializeActorsForPlay(FURL()); // TODO: check if necessary
// 	SimWorld->BeginPlay(); // TODO: check if necessary
//
...
//
// 		SimWorld->UpdateWorldComponents(false, true); // TODO: check if necessary
// 		SimWorld->UpdateLevelStreaming(); // TODO: check if necessary
// 		SimWorld->SendAllEndOfFrameUpdates(); // TODO: check if necessary
//
